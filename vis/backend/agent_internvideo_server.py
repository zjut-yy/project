...
def resolve_path(raw: str) -> Path:
    ...
    if p.is_absolute():
        ...
        if str(p).startswith('/backend/'):
            # Map /backend/dataset/... to REPO_ROOT/vis/backend/dataset/...
            rel_part = str(p).replace('/backend/', '', 1)
            mapped = (REPO_ROOT / 'vis' / 'backend' / rel_part).resolve()
            if mapped.exists():
    ...
    possible_paths = [
        ...
        (REPO_ROOT / 'backend' / cleaned).resolve(),
    ]
    ...
    default = (REPO_ROOT / 'vis' / 'backend' / cleaned).resolve()
